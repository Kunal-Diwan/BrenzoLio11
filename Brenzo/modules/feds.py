...
from Brenzo.modules.helper_funcs.misc import is_module_loaded, send_to_list
from Brenzo.modules.helper_funcs.chat_status import is_user_admin
...
def user_demote_fed(bot: Bot, update: Update, args: List[str]):
	...
	if is_user_fed_owner(fed_id, user.id):
		...
		if sql.search_user_in_fed(fed_id, user_id) == False:
			update.effective_message.reply_text(tld(chat.id, "feds_demote_target_not_admin"))
			return
